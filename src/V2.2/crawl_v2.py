...
def main():
    ...
    while True:
       ### （3）指定北京某个区域爬取该区域房屋信息
        region_param = easygui.enterbox("请输入北京市不同区域名称（比如：大兴）")
        easygui.msgbox('您需要爬取的区域名称是：'+region_param)
        # region_param = '大兴'                            #默认为大兴区域


       ### （4）按照区域顺序爬取北京城区所有房源信息
        if region_param == 'quit':
            print("退出爬取程序！")
            break
        elif region_param == '不限':
            ...
            while region_param in InforDict.keys():
                valid_info = 1
                break
            if valid_info == 1:
